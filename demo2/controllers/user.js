...
const { userSchema, formatDay } = require("../utils/validate");
...
exports.register = async(req, res) => {
    ...
    if (!validatePassword(password)) return res.send({ status: 400, message: 'Password minimum six characters, at least one letter, one number and one special character' })
    const result = await User.findOne({ where: { email } });
    if (result) return res.send({ status: 402, message: 'User exist' })
...
exports.login = async(req, res) => {
    ...
    if (result.is_ban == true) return res.status(400).json({ msg: 'User blocked' })
...
exports.updateInfo = async(req, res) => {
    ...
    if (!result) return req.status(404).json({ msg: 'User not found' });
...
exports.updateAvatar = async(req, res) => {
    const { userId } = req.params;
    if (!req.file) return res.status(400).json({ msg: 'No avatar provided' });
    const result = await User.findByPk(userId);
    if (!result) return res.status(404).json({ msg: 'User not found' });
...
exports.changePassword = async(req, res) => {
    ...
    if (!validatePassword(newPassword)) return res.send({ status: 400, message: 'Password minimum six characters, at least one letter, one number and one special character' })
    const result = await User.findByPk(userId);
    if (!result) return res.status(404).json({ msg: 'User not found' });
    const cp = await bcrypt.compare(oldPassword, result.password);
    if (!cp) return res.send({ status: 400, message: 'Current password wrong' });
...
exports.forgotPassword = async(req, res) => {
    ...
    try {
        ...
        if (!result) return res.send({ status: 404, message: 'User not found or not active yet' })
